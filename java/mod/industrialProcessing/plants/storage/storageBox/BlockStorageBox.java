...
public class BlockStorageBox extends BlockMachineRendered implements
		IDescriptionBlock, IGuiBlock {
	public static String UNLOCALIZED_NAME = "IP.Machine.StorageBox";
	private static IGuiBuilder guiBuilder = new GuiBuilderDefault(
			UNLOCALIZED_NAME).addInputSlotCluster(0, 9, 3,
	...
	public BlockStorageBox() {
		super(ConfigMachineBlocks.getBlockStorageBoxID(), Material.iron, 1F,
				Block.soundMetalFootstep, UNLOCALIZED_NAME,
				ISetupCreativeTabs.tabOreProcessing);
	}
	...
	public void breakBlock(World world, int x, int y, int z, int par5, int par6) {
		...
		TileEntityStorageBox box = (TileEntityStorageBox) world
				.getBlockTileEntity(x, y, z);
		for (int i = 0; i < box.getSizeInventory(); i++) {
			putStackInBox(box.getStackInSlot(i), nbtLoadedStack, i);
		}
		Random random = new Random();
		ItemTransfers.launch(nbtLoadedStack, world, x, y, z,
	...
	public void onBlockPlacedBy(World world, int x, int y, int z,
		...
		if (itemStack.stackTagCompound != null) {
			TileEntityStorageBox te = (TileEntityStorageBox) world
					.getBlockTileEntity(x, y, z);
	...
	public void registerIcons(IconRegister par1IconRegister) {
		this.icons[0] = par1IconRegister
				.registerIcon(INamepace.TEXTURE_NAME_PREFIX + "blockBox");
	}
	...
	public MovingObjectPosition collisionRayTrace(World par1World, int par2,
		...
		int l = ((IRotateableEntity) par1World.getBlockTileEntity(par2, par3,
				par4)).getForwardDirection().ordinal();
		if (l == 2) {
	...
	public static ItemStack putStackInBox(ItemStack stack, ItemStack box,
			int slot) {
		if (box.itemID == ConfigMachineBlocks.getBlockStorageBoxID()
				&& slot >= 0 && slot < STORAGE_SIZE && stack != null
				&& stack.itemID != ConfigMachineBlocks.getBlockStorageBoxID()) {
			...
			if (slotStack == null) {
				...
				if (slotStack.stackSize < slotStack.getMaxStackSize()) {
					int transferCount = slotStack.getMaxStackSize()
							- slotStack.stackSize;
	...
	public static ItemStack getStackFromBox(ItemStack box, int slot, int amount) {
		if (box != null
				&& box.itemID == ConfigMachineBlocks.getBlockStorageBoxID()
				&& box.stackTagCompound != null && slot >= 0
				&& slot < STORAGE_SIZE) {
	...
	private static boolean setStackInSlot(ItemStack box, int slot,
			ItemStack stack) {
		NBTTagList list;
	...
	private static ItemStack getStackInSlot(ItemStack box, int slot) {
		if (box.stackTagCompound.hasKey("Slots")) {
			...
			if (slot < list.tagCount()) {
				...
				boolean isNullTag = tag.hasKey("NullStack")
						&& tag.getBoolean("NullStack");
				if (isNullTag)
					return null;
