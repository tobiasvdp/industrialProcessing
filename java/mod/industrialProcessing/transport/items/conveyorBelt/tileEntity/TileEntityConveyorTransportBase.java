...
import mod.industrialProcessing.client.rendering.tileEntity.animation.conveyors.TileConveyorSyncHandler;
...
public abstract class TileEntityConveyorTransportBase extends TileEntityConveyorConnectionsBase implements IConveyor {
	...
	private boolean outputStack(MovingItemStack stack) {
		...
		if (isOutput(outputLocal)) {
			ForgeDirection direction = getExternalDirection(outputLocal);
	...
	private boolean isOutput(LocalDirection local) {
		...
		ForgeDirection direction = getExternalDirection(local);
	...
	public void readFromNBT(NBTTagCompound nbt) {
		...
		NBTTagList nbttaglist = nbt.getTagList("Stacks", 10);
