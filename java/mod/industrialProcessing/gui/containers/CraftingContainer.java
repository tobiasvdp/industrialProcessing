...
public class CraftingContainer extends LayoutContainer {
	...
	public ItemStack slotClick(int par1, int par2, int par3, EntityPlayer par4EntityPlayer) {
		...
		if (par1 >= 0 && getSlot(par1).getHasStack() && getSlot(par1).getStack().stackSize == 0) {
			...
			if (par1 >= 0 && !getSlot(par1).getHasStack() && par4EntityPlayer.inventory.getItemStack() == null) {
				ItemStack stack2 = null;
				for (int i = 0; i < getSizeInventory(); i++) {
					if (getSlot(i) instanceof SlotBase && !( getSlot(i) instanceof SlotOutput) && getSlot(i).getHasStack() && getSlot(i).getStack().stackSize == 0) {
						stack2 = getSlot(i).getStack();
						getSlot(i).putStack(null);
					}
				}
				getSlot(par1).putStack(stack2);
			} else {
