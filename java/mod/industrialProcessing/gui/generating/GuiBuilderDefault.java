...
public class GuiBuilderDefault implements IGuiBuilder {
	...
	public GuiBuilderDefault addBufferSlotCluster(int startSlot, int count, int size, Orientation orientation) {
		this.bufferSlots.add(new SlotClusterReference(startSlot, count, size, orientation));
		return this;
	}
	...
	public GuiLayoutContainer getGuiContainer(LayoutContainer container, TileEntity tileEntity) {
		...
		if (powerRef != null) {
			LayerPanel powerPanel = new LayerPanel();
			DefaultPower.setup(powerRef, guiContainer, powerPanel, Alignment.MAX);

			Decorator powerDecorator = Decorator.createFlyoutDecorator();
			powerDecorator.margin = new Thickness(5, 0, 5, 0);
			powerDecorator.setChild(powerPanel);
			flyout.addChild(powerDecorator);
		}

		if (durabilityRef != null) {

			StackPanel durabilityPanel = new StackPanel();
			DefaultDurability.setup(durabilityRef, guiContainer, durabilityPanel);

			Decorator durabilityDecorator = Decorator.createFlyoutDecorator();
			durabilityDecorator.margin = new Thickness(5, 0, 5, 0);
			durabilityDecorator.setChild(durabilityPanel);
			flyout.addChild(durabilityDecorator);
		...
		if (!bufferSlots.isEmpty()) {
			StackPanel bufferStack = new StackPanel();
			bufferStack.orientation = Orientation.HORIZONTAL;
			bufferStack.margin = new Thickness(5, 0, 0, 0);
			DefaultSlots.setup(this.bufferSlots, container, bufferStack, Alignment.CENTER);
			verticalStack.addChild(bufferStack);
		}
	...
	public LayoutContainer getContainer(InventoryPlayer player, TileEntity tileEntity) {
		...
		if (!bufferSlots.isEmpty()) {
			DefaultSlots.setup(this.bufferSlots, container, tileEntity, true);
		}
