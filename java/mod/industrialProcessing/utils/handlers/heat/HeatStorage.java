...
public class HeatStorage {
	...
	public static boolean onIsValidInput(Item item) {
		if (GameRegistry.getFuelValue(new ItemStack(item, 1, 0)) != 0) {
			return true;
		}
		return false;
	}

	public static void ConsumeFuel(World world,IHeatStorage heat, IInventory machineSlot,IInventory inventory, ItemStack stack, int slot) {
		if (stack != null) {
			int val = GameRegistry.getFuelValue(stack);
			if (val != 0 && stack.stackSize > 0) {
				machineSlot.decrStackSize(slot, 1);
				inventory.markDirty();
