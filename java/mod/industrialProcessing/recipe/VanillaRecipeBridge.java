	...
	public static Recipe getRecipeFromVanilla(IRecipe recipe) {
		...
		if (recipe instanceof RecipeFireworks) {
			...
			for (int i = 0; i < recipeVanilla.recipeItems.length; i++) {
				...
				if (itemStack != null)
					ipRecipe.inputs[i] = new RecipeInputSlot(i, itemStack.getItem(), itemStack.getItemDamage(), RecipeSlotType.INVENTORY, itemStack.stackSize);
			}
		...
		if (recipe.getRecipeOutput() != null) {
			ipRecipe.outputs = new RecipeOutputSlot[] { new RecipeOutputSlot(9, recipe.getRecipeOutput().getItem(), recipe.getRecipeOutput().getItemDamage(), RecipeSlotType.INVENTORY, recipe.getRecipeOutput().stackSize, recipe.getRecipeOutput().stackSize, 0) };
