...
public class ItemWoodenBucketFilled extends ItemIPBucket {
	...
	public ItemWoodenBucketFilled(Block waterstill) {
		super(waterstill);
		isFull = waterstill;
	}
	...
	public Item getContainerItem() {
		return IndustrialProcessing.itemWoodenBucket;
	}
			...
			if (movingobjectposition.typeOfHit == MovingObjectType.BLOCK) {
				...
				if (this.isFull == null) {
					...
					if (par2World.getBlock(i, j, k).getMaterial() == Material.water && par2World.getBlockMetadata(i, j, k) == 0) {
						...
						if (!par3EntityPlayer.inventory.addItemStackToInventory(new ItemStack(Items.water_bucket))) {
							par3EntityPlayer.dropItem(Items.water_bucket, 1);
						}
					...
					if (par2World.getBlock(i, j, k).getMaterial() == Material.lava && par2World.getBlockMetadata(i, j, k) == 0) {
						...
						if (!par3EntityPlayer.inventory.addItemStackToInventory(new ItemStack(Items.lava_bucket))) {
							par3EntityPlayer.dropItem(Items.lava_bucket, 1);
						}
	...
	public boolean tryPlaceContainedLiquid(World par1World, int par2, int par3, int par4) {
		if (this.isFull == null) {
			...
			if (!par1World.isAirBlock(par2, par3, par4) && !flag) {
				...
				if (par1World.provider.isHellWorld && this.isFull.equals(Blocks.water)) {
					...
					this.isFull.onNeighborBlockChange(par1World, par2, par3, par4, null);
				}
