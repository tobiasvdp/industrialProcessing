		...
		case horizontal:
			...
			if (yHor > 1) {
				for (int i = 0; i < (yHor - 1); i++) {
					for (int j = 0; j < 3; j++)
						slots[count++] = new SlotOutput(null, startIndex++, 18 * j, rect.height/2 - yHor * 9 + i * 18);
				}
				for (int i = 0; i < xHor; i++) {
					slots[count++] = new SlotOutput(null, startIndex++, rect.width/2 - xHor * 9 + i * 18, rect.height/2 - yHor * 9 + (yHor - 1) * 18);
				}
			} else {
		...
		case vertical:
			...
			if (xVer > 1) {
				for (int i = 0; i < (xVer - 1); i++) {
					for (int j = 0; j < 3; j++)
						slots[count++] = new SlotOutput(null, startIndex++, rect.width/2 - xVer * 9 + i * 18, 18 * j);
				}
				for (int i = 0; i < yVer; i++) {
					slots[count++] = new SlotOutput(null, startIndex++, rect.width/2 - xVer * 9 + (xVer - 1) * 18, rect.height/2 - yVer * 9 + i * 18);
				}
			} else {
